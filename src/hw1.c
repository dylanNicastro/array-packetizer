                          ...
                          unsigned int compression_scheme, unsigned int traffic_class)
{
    ...
    int packets_needed = packets_len;
    int max_ints_per_packet = max_payload/4;
    // if amount of integers to be stored is less than the amount of integers that can be stored:
    if (array_len < packets_len*max_ints_per_packet) {
        packets_needed=ceil((double)array_len/(double)max_ints_per_packet);
    }
    for (int i = 0; i < packets_needed; i++) {
        int packet_payload_size;
        // if this is the last packet && the integers still needed to be stored is less than the amount that can be stored:
        if ((i == packets_needed-1) && (array_len-i*max_ints_per_packet < max_ints_per_packet)) {
            // payload size = the amount of integers needed to be stored times 4 (to get bytes)
            packet_payload_size = (array_len-i*max_ints_per_packet)*4;
        }
        else {
